...
def update_clock():
    ...
    canvas.create_line(WIDTH / 2, HEIGHT / 2, hour_x, hour_y, fill="Red", width=6)
    ...
    canvas.create_line(WIDTH / 2, HEIGHT / 2, minute_x, minute_y, fill="white", width=4)

    # Draw second hand
    second_angle = second * math.pi / 30 - math.pi / 2
    second_x = WIDTH / 2 + 0.6 * WIDTH / 2 * math.cos(second_angle)
    second_y = HEIGHT / 2 + 0.6 * WIDTH / 2 * math.sin(second_angle)
    canvas.create_line(WIDTH / 2, HEIGHT / 2, second_x, second_y, fill="Grey", width=2)
